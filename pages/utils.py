...
def restructure_metadata(df_metadata):
    ...
    for i in range(len(species_selected)):
        ...
        for j in range(len(carbon_source_selected)):
            ...
            for i_0 in range(len(projects_present)):
                cur_project = projects_present[i_0]
                projects_filtered_metadata = cs_filtered_metadata[cs_filtered_metadata["project"] == cur_project]
                unique_cs_concs = projects_filtered_metadata["cs_conc"].unique()
                cur_cs_concs.append(unique_cs_concs)
                lg_project_conc_replicates = []
                for k in range(len(unique_cs_concs)):
                    lg_selected_conc = projects_filtered_metadata[projects_filtered_metadata["cs_conc"] == unique_cs_concs[k]]["linegroup"].unique()
                    lg_project_conc_replicates.append(lg_selected_conc)
                lg_conc_replicates.append(lg_project_conc_replicates)
            cur_species_concs.append(cur_cs_concs)
            cur_sp_lg_replicates.append(lg_conc_replicates)
...
def plot_data(df_merged,filtered_metadata,color_by,plot_replicates,fig_layout):
    ...
    for i in range(len(species_selected)):
        ...
        for j in range(len(carbon_source_selected)):
            ...
            for i_0 in range(len(projects_present)):
                if(len(concentrations_present[i][j][i_0]) == 0):
                    continue
                for k in range(len(concentrations_present[i][j][i_0])):
                    cur_conc = concentrations_present[i][j][i_0][k]
                    cur_lgs = lg_replicates[i][j][i_0][k]
