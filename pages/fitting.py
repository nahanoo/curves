...
# Dash layout
dash.register_page(__name__, path="/Fitting", name="Fitting")  # '/' is home page
...
def fit_parameters(chosen_projects, chosen_carbon_sources, chosen_species,nclicks):
    ...
    if(chosen_projects == "Select Project" or chosen_projects == None or chosen_carbon_sources == "Select Carbon Source" or chosen_carbon_sources == None or chosen_species == "Select Species" or chosen_species == None):
        return no_update, False
    ...
    if len(filtered_metadata) == 0:
        return dbc.Alert("No data found for the selected conditions", color="warning"), True

    parsed_data_dir = "export"
    df_merged = utils.load_data_from_metadata(filtered_metadata, pooled_df_joint_metadata, parsed_data_dir)

    return dbc.Alert("Data found but fitting still under construction", color="warning"), True
